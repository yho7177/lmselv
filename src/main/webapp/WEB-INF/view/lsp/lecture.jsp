...
<script type="text/javascript">
	...
	function fListLsp(currentPage) {
		...
		var resultCallback = function(data) {
			flistLspResult(data, currentPage);
		};
		//Ajax실행 방식 (사용자 정의함수)
		//callAjax("Url",type,return,async or sync방식,넘겨준거,값,Callback함수 이름)
		callAjax("/Lsp/listLsp.do", "post", "text", true, param, resultCallback); 
	}
	
	
	
	/** 강의  조회 콜백 함수 */
	function flistLspResult(data, currentPage) {
		...
		if(data.usertype == "C"){
			...
			// 신규 목록 생성
			$("#listLsp").append(data.listLspStudent);
		...
		var paginationHtml = getPaginationHtml(currentPage, countListComnGrpCod, pageSizeComnGrpCod, pageBlockSizeComnGrpCod, 'fListLsp');
		console.log("paginationHtml : " + paginationHtml);
		//swal(paginationHtml);
		$("#countListLsp").empty().append( paginationHtml );
	...
	/** 강의  단건 조회 */
	function fSelectLsp(ci_no) {
		
		var param = { ci_no : ci_no };
		
		var resultCallback = function(data) {
			fSelectLspResult(data);
		};
		
		callAjax("/Lsp/selectLsp.do", "post", "json", true, param, resultCallback);
	}
	
	
	/** 강의  단건 조회 콜백 함수*/
	function fSelectLspResult(data) {
	...
	function fSaveLsp() {
		...
		callAjax("/Lsp/saveLsp.do", "post", "json", true, $("#myForm").serialize(), resultCallback);
	}
	
	
	/** 강의  저장 콜백 함수 */
	function fSaveLspResult(data) {
		...
		swal("정말 삭제하겠습니까? \n삭제시 복구불가합니다.", {
			  buttons:{
				...
				case "yes":
					...
					var resultCallback = function(data) {
					fDeleteLspResult(data);
					};
				
					callAjax("/Lsp/deleteLsp.do", "post", "json", true, $("#myForm").serialize(), resultCallback);
	...
	/** 강의  삭제 콜백 함수 */
	function fDeleteLspResult(data) {
					...
					<li class="contents">
						...
						<div class="content">
							...
							<p class="Location">
								...
								<span class="btn_nav bold">강의정보</span> 
								<span class="btn_nav bold">강의목록</span> 
								<a href="../Lsp/lectureMgt.do" class="btn_set refresh">새로고침</a>
							</p>
							...
							<p class="conTitle">
								...
								<span class="fr"> 
									...
									<a class="btnType blue" href="javascript:fPopModalLsp();" name="modal">
									<span>신규등록</span>
								</a>
								</span>
							</p>
							...
							<div class="divComGrpCodList">
								<table class="col">
									...
									<tbody id="listLsp"></tbody>
								</table>
							</div>

							<div class="paging_area" id="countListLsp"></div>
						...
						<tbody>
							...
							<tr>
								<td style="display:none;">
									<input type="hidden" class="inputTxt p100" id="loginId" name="loginId" value="${loginId}"/>
								</td>
							</tr>
							<tr>
							...
							<tr>
								<th scope="row">출석</th>
								<td colspan="6">
									<textarea type="text" class="inputTxt p100" id="ci_attend" name="ci_attend"></textarea>
								</td>
							</tr>
							<tr>
								<th scope="row">강의목표</th>
								<td colspan="6">
									<textarea type="text" class="inputTxt p100" id="ci_goal" name="ci_goal"></textarea>
								</td>
							</tr>
							<tr>
								<th scope="row">강의계획</th>
								<td colspan="6">
									<textarea type="text" class="inputTxt p100" id="ci_plan" name="ci_plan"></textarea>
								</td>
							</tr>
							<tr>
								<th scope="row">강의설명</th>
								<td colspan="6">
									<textarea type="text" class="inputTxt p100" id="ci_content" name="ci_content"></textarea>
								</td>
							</tr>
						</tbody>
					</table>
