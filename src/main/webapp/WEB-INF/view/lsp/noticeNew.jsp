...
<script type="text/javascript">
	...
	function selectNoticeList(currentPage) {
		...
		callAjax("/lsp/noticeList.do", "post", "text", true, param, resultCallback);
	...
	function fNoticeModal(ntNo) {
		...
		if (ntNo == null || ntNo == "") {
			...
			// Tranjection type 설정
			$("#action").val("U"); // update
			fdetailModal(ntNo); //번호로 -> 공지사항 상세 조회 팝업 띄우기
		}

	}

	/*공지사항 상세 조회*/
	function fdetailModal(ntNo) {
// 		alert("공지사항 상세 조회  ");
		...
		callAjax("/lsp/detailNotice.do", "post", "json", true, param,resultCallback2);
		//alert("공지사항 상세 조회  22");
	}
	...
	function frealPopModal(object) {
		...
		if (object == "" || object == null || object == undefined) {
			...
			$("#btnUpdateNotice").hide(); // 삭제버튼 숨기기
			$("#write_date_t").hide();
			$("#btnSaveNotice").show(); // 삭제버튼 숨기기
			...
			$("#ntTitle").val(object.ntTitle);
			$("#ntContent").val(object.ntContent);
			//////object.noticeNo

			$("#ntNo").val(object.ntNo); // 중요한 num 값도 숨겨서 받아온다. 

			$("#write_date_t").show();
			
			$("#btnSaveNotice").hide(); // 삭제버튼 숨기기
			if(usertype == 'C'){
				$("#btnDeleteNotice").hide(); // 삭제버튼 숨기기
				$("#btnUpdateNotice").hide(); // 삭제버튼 숨기기
			} else {
				$("#btnDeleteNotice").show(); // 삭제버튼 숨기기
				$("#btnUpdateNotice").show(); // 삭제버튼 숨기기
			}
			...
			if( object.file_no == "" || object.file_no == null || object.file_no == undefined) {
				...
				filehtml = "<a href=\"javascript:downloadNtFile('" + object.ntNo + "')\" >" +  object.file_name + "</a>";
	...
	// 파일다운로드
	function downloadNtFile(ntNo) {
	   
		var params = "";
		params += "<input type='hidden' name='ntNo' value='"+ ntNo +"' />";
	
		jQuery("<form action='/lsp/downloadNtFile.do' method='post'>"+params+"</form>").appendTo('body').submit().remove();
	...
	function fSaveNotice() {
		...
		callAjaxFileUploadSetFormData("/lsp/noticeSave.do", "post", "json", true, fileData, resultCallback3);
		...
		swal("정말 삭제하겠습니까? \n삭제시 복구불가합니다.", {
			  buttons:{
				...
				case "yes":
					...
					callAjaxFileUploadSetFormData("/lsp/noticeDelete.do", "post", "json", true, fileData, resultCallback3);
...
<body>
	...
	<form id="myNotice" action="" method="">
		...
		<input type="hidden" name="action" id="action" value=""> <input
			type="hidden" id="swriter" value="${loginId}">
						...
						<div class="content">
							...
							<p class="conTitle">
								<span>공지 사항 </span> <span class="fr"> <c:set
									...
									href="javascript:fNoticeModal();" name="modal" id="btnInsertNotice"> <span>신규등록</span></a>
								</span>
							</p>
					...
					<table class="row">
						...
						<tbody>
							<tr>
								...
								<td colspan="3"><input type="text" class="inputTxt p100" name="loginID"
									id="loginID" /></td>
							</tr>
							<tr id="write_date_t">
								<th scope="row">작성일</th>
								<td colspan="3"><input type="text" class="inputTxt p100" name="write_date" id="write_date" /></td>
							</tr>
							<tr>
								<th scope="row">제목 <span class="font_red">*</span></th>
								<td colspan="3"><input type="text" class="inputTxt p100"
									name="ntTitle" id="ntTitle" /></td>
							</tr>
							<tr>
								<th scope="row">내용</th>
								<td colspan="3"><textarea class="inputTxt p100"
										name="ntContent" id="ntContent">
								</textarea></td>
							</tr>
