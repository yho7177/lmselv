					...
					<div class="content">
						...
						<p class="Location">
							...
							<span class="btn_nav bold">학습관리</span> <span class="btn_nav bold">시험
								결과 조회 </span> <a href="/lmg/exsubjectMgt.do" class="btn_set refresh">새로고침</a>
						</p>
								...
								<tbody>
									...
									<c:forEach var="i" items="${lst }" varStatus="status">
										<tr onclick="testInfo(${i.ci_no}, ${i.test_no })">
											...
											<td>${taker[status.index] }</td>
											<td>${nonTaker[status.index] }</td>
											<td>${taker[status.index] - nonTaker[status.index] }</td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
