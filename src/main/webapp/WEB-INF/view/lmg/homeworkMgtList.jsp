...
<c:forEach items="${listHwMgtModel}" var="list">
	<tr>
		...
		<td>
			<a href="javascript:fListHwSub(1, '${ list.hw_no}', '${ list.hw_name}', '${list.ci_name}', '${list.ci_no}');"><span>${list.hw_name}</span></a>
		</td>
		<td><fmt:formatDate  value="${list.hw_date}" pattern="yyyy-MM-dd"/></td>
		<td>
			<c:if test="${usertype == 'C' }">
				<a class="btnType3 color1" href="javascript:fHwMgtPopModal('${ list.hw_no}');"><span>조회</span></a>
			</c:if>
			<c:if test="${usertype == 'D' }">
				<a class="btnType3 color1" href="javascript:fHwMgtPopModal('${ list.hw_no}');"><span>수정</span></a>
			</c:if>
		</td>
	</tr>
